class "Player" {
	...
	loadSongFromFile = function (self, file)
		...
		if success then
			...
			love.window.showMessageBox("Error", --[["It is not a valid/supported MIDI file."]] "an error occurred while loading the midi file: " .. song, "error")
		end
				...
				if self.isMovingForward then
					for j = self.lastPlayedEventIDs[i]+1, #track:getRawEvents() do
						...
						if time >= event:getTime() then
							...
							if math.floor(event:getType() / 16) == 0x9 and event:getMsg2() > 0 then
